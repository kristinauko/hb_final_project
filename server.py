...
def create_json():
    ...
    print("1 AMAZON ID FROM JSON: ********************************", amazon_id)
    #product_quotes_list = Quote.query.filter(Product.amazon_id == amazon_id).all()
    product_quotes_list = Quote.query.join(Product, Quote.product_id==Product.product_id).filter(Product.amazon_id==amazon_id).all()
    print("2 PRODUCT QUOTES LIST: ********************************", product_quotes_list)
    date_time_list = []
    price_list = []
    quotes_dictionary = {}
